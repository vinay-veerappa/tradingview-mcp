...
- Build (2026-08-29): all 8 queue steps landed → tag `v2.3.0-surface-v3`. Build-order deltas worth knowing:
  ...
  - **ADR 0001 ACCEPTED + IMPLEMENTED** (this working tree, after the draft above): all three verification hooks pass. Registry gate (`op()` requires `meta.mutation_adr` for non-GET/non-read bindings; method allowlist POST/PATCH/DELETE; destructive/open-world never HTTP-bindable even with the meta key); server gate (`TV_GATEWAY_MUTATIONS === 'on'` exact-match via the exported pure `mutationsAuthorized(env, remoteAddress)`; non-loopback → 403 `http_forbidden`; loopback flag-off → 404 `http_mutations_disabled`; JSON bodies with a 1 MB cap; 405 + `Allow` on wrong methods); six paper routes bound (`POST /paper/connect`, `POST /paper/orders` with the **mandatory** `client_order_id` adapter guard, `POST /paper/orders/cancel`, `PATCH /paper/orders/modify`, `POST /paper/positions/close`, `PATCH /paper/brackets`). Tests: registry meta-gate matrix (14), gateway posture (14, incl. 404-default + gate-on flows + exact-match flag), HTTP idempotency replay (`deduplicated: true`, single placement, missing-key refusal). 481/481 battery, lint 0 errors. **With `TV_GATEWAY_MUTATIONS` unset the gateway is byte-for-byte as read-only as before.**
  - What remains: nothing in the v3 catalog — P2-1..P2-19 are complete (P2-16 ships inside payloads as noted; P2-10 shipped; ADR 0001 implemented). Future work is new scope, not this plan's backlog.