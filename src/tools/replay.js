...
export function registerReplayTools(server, _deps = null) {
  ...
  op('replay_start', 'Start bar replay mode, optionally at a specific date and exact time', {
  ...
  toolFromRegistry(server, 'replay_start');

  op('replay_step', 'Advance one bar in replay mode', {},
    A.MUTATE_IDEMPOTENT, async () => {
      try { return jsonResult(await core.step(d())); }
      catch (err) { return errorResult(err); }
    });
  toolFromRegistry(server, 'replay_step');

  op('replay_autoplay', 'Toggle autoplay in replay mode, optionally set speed', {
  ...
  toolFromRegistry(server, 'replay_autoplay');

  op('replay_stop', 'Stop replay and return to realtime', {},
    A.MUTATE_IDEMPOTENT, async () => {
      try { return jsonResult(await core.stop(d())); }
      catch (err) { return errorResult(err); }
    });
  toolFromRegistry(server, 'replay_stop');

  op('replay_trade', 'Execute a trade action in replay mode (buy, sell, or close position)', {
  ...
  toolFromRegistry(server, 'replay_trade');

  op('replay_status', 'Get current replay mode status', {},
    A.READ, async () => {
      try { return jsonResult(await core.status(d())); }
      catch (err) { return errorResult(err); }
    });
  toolFromRegistry(server, 'replay_status');
}