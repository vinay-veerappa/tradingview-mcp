...
export function registerIndicatorTools(server) {
  op('indicator_set_inputs', 'Change indicator/study input values (e.g., length, source, period)', {
  ...
  toolFromRegistry(server, 'indicator_set_inputs');

  op('indicator_toggle_visibility', 'Show or hide an indicator/study on the chart', {
    entity_id: z.string().describe('Entity ID of the study (from chart_get_state)'),
  ...
  toolFromRegistry(server, 'indicator_toggle_visibility');

  op('indicator_search', 'Search TradingView\'s Indicators dialog for indicators, strategies, and community/public scripts by keyword. Returns matching titles grouped by section (Technicals, Community, My scripts, etc.).', {
  ...
  toolFromRegistry(server, 'indicator_search');

  op('indicator_add', 'Search the Indicators dialog and add a result to the chart by name. Works for strategies and community scripts, not just built-ins. Returns the new study entity_id.', {
  ...
  toolFromRegistry(server, 'indicator_add');
}